...
class BlockchainController {
  ...
  async createBlockchain(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error creating blockchain", error: error.message });
    }
  }
  ...
  async getBlockchain(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error retrieving blockchain", error: error.message });
    }
  }
  ...
  async addBlock(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error adding block", error: error.message });
    }
  }
  ...
  async validateBlockchain(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error validating blockchain", error: error.message });
    }
  }
      ...
      for (const block of chain) {
        for (const tx of block.transactions) {
          ...
          const emailMatches =
            ...
            (tx.fromAddress &&
              tx.fromAddress.toLowerCase() === email.toLowerCase()) ||
          ...
          if (emailMatches && isStudentRecord) {
            ...
            if (!studentData || new Date(tx.timestamp) > new Date(timestamp)) {
              ...
              logger.info(
                `Found student record in blockchain block ${block.index}, timestamp ${tx.timestamp}`
              );
            }
          }
        }
      ...
      for (const tx of pendingTransactions) {
        ...
        if (emailMatches && isStudentRecord) {
          ...
          if (!studentData || new Date(tx.timestamp) > new Date(timestamp)) {
            ...
            logger.info(
              `Found student record in pending transactions, timestamp ${tx.timestamp}`
            );
          }
        }
      }
  ...
  async getUserTransactions(req, res) {
    try {
      ...
      logger.info(
        `Getting transactions for user with email: ${email}, studentId: ${studentId}`
      );
